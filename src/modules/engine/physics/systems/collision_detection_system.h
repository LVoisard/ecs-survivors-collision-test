...
namespace physics::systems {
    ...
    inline void collision_detection_non_static_entity_system(flecs::iter &self_it, size_t self_id,
                                           const core::Position2D &pos,
                                           const Collider &collider) {
        std::vector<CollisionRecord> collisions;
        std::vector<CollisionRecord> events;
        ...
        visible_query.each(
            [&](flecs::iter& other_it, size_t other_id, const core::Position2D &other_pos,
                ...
                if (CheckCollisionRecs(self_rec, other_rec)) {
                    stage_world.entity().set<CollisionRecord>({self, other});
                }
            });
    }
    ...
    inline void collision_detection_static_system(flecs::iter &self_it, size_t self_id, CollisionRecordList &list,
                                           const core::Position2D &pos,
                                           const Collider &collider) {
        std::vector<CollisionRecord> collisions;
        std::vector<CollisionRecord> events;
